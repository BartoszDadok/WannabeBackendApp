...
export const getTypeScriptFlashcards = async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    const userLanguages = req.user.languages;
    if (!userLanguages.includes("typescript")) return res.status(403).json({
        success: false,
        errors: ["Unauthorized access to flashcards!"],
    });
    try {
...
export const postAddFlashcards = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!updatedFlashcards)
            if (!foundLanguage) return res.status(400).json({
                success: false,
                message: "Something went wrong with updating flashcard",
            });
        return res.status(201).json({ success: true, message: "Flashcards created" });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: "Internal Server Error" });
    }
...
export const postAddLanguage = async (req: Request, res: Response, next: NextFunction) => {
    const language = req.body.language;
...
export const postUpdateFlashcards = async (req: Request, res: Response, next: NextFunction) => {
    const language = req.body.language;
    const newFlashcard = req.body.newFlashcard;
    const id = newFlashcard.id;

    if (!language && !id && !newFlashcard.question && !(newFlashcard.flashcard.length === 2)) {
        return res.status(400).json({ success: false, message: "Wrong request body" });
    }
    try {
        const foundLanguage = await LanguagesModel.findOne({ language: language });
        if (!foundLanguage) return res.status(400).json({ success: false, message: "Language not found" });
        ...
        const updatedFlashcards = await LanguagesModel.updateOne({ language: req.body.language }, { flashcards: updatedFl });
        if (!updatedFlashcards)
            if (!foundLanguage) return res.status(400).json({
                success: false,
                message: "Something went wrong with updating flashcard",
